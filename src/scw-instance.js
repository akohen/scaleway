...
export default class Instance extends ScalewayBase {
  constructor(args) {
  ...
  async deleteServer({ zone = this.defaults.zone, id }) {
    try {
  ...
  async createIP({
    ...
    try {
      const response = await this.api.post(`${zone}/ips`, { organization });
  ...
  async deleteIP({ zone = this.defaults.zone, id }) {
    try {
  ...
  async getIP({ zone = this.defaults.zone, id }) {
    try {
  ...
  async listIPs({ zone = this.defaults.zone } = {}) {
    try {
  ...
  // Volumes
  async deleteVolume({ zone = this.defaults.zone, id }) {
    try {
  ...
  // Other
  async action({ zone = this.defaults.zone, id, action }) {
    try {
      const response = await this.api.post(`${zone}/servers/${id}/action`, { action });
  ...
  // Metadata
  async setCloudInit({ zone = this.defaults.zone, id, cloudInit }) {
    try {
      await this.api.patch(
        `${zone}/servers/${id}/user_data/cloud-init`,
        cloudInit, { headers: { 'Content-Type': 'text/plain' } },
      );
    } catch (err) {
